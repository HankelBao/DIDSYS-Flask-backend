...
class Document(EmbeddedDocument):
    def save(self):
        if mongo.db.__getattr__(self.__table__).find_one({self["__primary__"]: self[self["__primary__"]]}):
            mongo.db.__getattr__(self.__table__).update({self["__primary__"]: self[self["__primary__"]]}, self)
        else:
